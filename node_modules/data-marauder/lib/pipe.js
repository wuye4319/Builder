...
// constructor
class InitJs {
  async getdata(browser, type, url, rules, process, mobile, selfbrowser) {
      ...
      try {
        let waitcont = async (index) => {
          ...
          if (!cont.cont) {
            ...
            logger.myconsole((selfbrowser ? 'Backup service!' : 'Web page!') + process)
            cont.status = 'success'
            t = Date.now() - t
            mylogstr.Loadingtime = (t / 1000).toString() + ' s'
            logger.myconsole('Loading time : ' + mylogstr.Loadingtime)
          }
        }
          ...
          if (!intercept) {
            ...
            if (filter) {
              try {
                ...
                resolve(cont)
                logger.myconsole('Get filter failed!' + process + e)
              }
        ...
        cont.html = await page.content()
        // close page when analysis is done
        await page.close()
        if (!intercept) {
          selfbrowser ? logger.myconsole('Self browser product is missing! ' + process) : logger.myconsole('Product is missing! ' +
            process)
        ...
        logger.myconsole('System error! Or page timeout!')
        await page.close()
      }
    })
  }
