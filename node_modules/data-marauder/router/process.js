...
class ctrl {
  async filter(myurl, rout, datatype, mobile) {
    // filter
    ...
    if (internumb > 15) {
      ...
      if (hasurl) {
        ...
        if (hascache) {
          ...
          result = await this.getdatatype(rout, myurl, datatype, mobile)
        }
  ...
  async getdatatype(rout, url, type, mobile) {
    switch (type) {
      case 'pipe':
        return proxy.pipedata(rout, url, internumb, mobile)
      case 'url':
